...
void	child2(t_pipex *pip)
{
	if (dup2(pip->pipefd[0], STDIN_FILENO) < 0)
		perror_exit(pip, "dup2 pipe read");
	if (dup2(pip->outfile, STDOUT_FILENO) < 0)
		perror_exit(pip, "dup2 outfile");
	close(pip->pipefd[1]);
	close(pip->pipefd[0]);
	close(pip->infile);
	close(pip->outfile);
	execve(pip->cmd_path2, pip->cmd2, pip->envi);
	perror_exit(pip, "execve cmd2");
}

void	execute(t_pipex *pip)
{
	if (pipe(pip->pipefd) < 0)
		perror_exit(pip, "pipe");
	pip->pid1 = fork();
	if (pip->pid1 < 0)
		perror_exit(pip, "fork");
	if (pip->pid1 == 0)
		child1(pip);
	pip->pid2 = fork();
	if (pip->pid2 < 0)
...
int	main(int ac, char **av, char **envi)
{
	...
	if (ac != 5)
		perror_exit(&pip, "Wrong number of argument");
